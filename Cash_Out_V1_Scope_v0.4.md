...
![Version](https://img.shields.io/badge/version-v0.4-blue) ![Status](https://img.shields.io/badge/status-Draft-orange) ![Date](https://img.shields.io/badge/date-2026--04--27-lightgrey) ![Module](https://img.shields.io/badge/module-Sportsbook%20Core-informational)

| Program Owner | Date | Version | Status |
|:---|:---|:---|:---|
| _To be assigned_ | 27 Apr 2026 | v0.4 | Draft |
...
Punters with pending Bets currently have no way to lock in a partial return before the event settles. They watch a winning position drift back to neutral, or absorb the full loss on a losing position, with no exit. Customer feedback consistently flags Cash Out as the single most-requested missing feature, and Punters churning to competitors cite "couldn't cash out" as a top-3 reason. The friction applies equally to Bets placed long before the event and Bets placed during the event — Punters expect early settlement to be available throughout the entire window between placement and grading, across every market they wagered on.

#### 🏢 Business Problem

Three intersecting drivers:
1. **Competitive disadvantage.** Every major sportsbook offers Cash Out across the standard market set. Absence is visible in competitor acquisition messaging and in retention churn data.
...
- **Competitive scan** — every major regulated sportsbook offers Cash Out across the standard market set. Several market it as a primary acquisition feature.
...
|:---|:---|
| **Frequency** | Every active Punter session with a pending Bet on an enabled league / market sees the Cash Out icon. Coverage of every enabled market — including those that reference a numeric line — materially expands the addressable cashout volume vs a partial rollout. |
...
**Active web-sportsbook Punters with pending Bets on sports / leagues / markets enabled in the back office.** This includes Punters whose Bets reference a numeric line and Punters whose Bets do not. The launch tournament is the framing cohort. This segment is already demonstrated to value Cash Out through competitor-product analytics and direct customer feedback.

#### 👥 Secondary Audience

| Stakeholder | Why They Care |
|:---|:---|
| **Trading / Risk** | Owns the vig rate table, admin fee values, sport / league enablement decisions, and the chosen approach for handling line moves on markets where the reference line can shift. Real-time liability visibility is required to hedge. |
...
| **Handle on enabled leagues** (across every enabled market) | _TBD — Analytics to provide pre-launch baseline_ | **+5–10% lift** in launch tournament window vs cohort baseline _(target needs Trading + Analytics confirmation)_ |
...
| Cashout coverage by market | Business | ✅ | % of eligible pending Bets across every enabled market where a live cashout offer is available at any given moment |
...
- Customer Support ticket-handling playbook (new "Cashed Out" status, new transaction type, push-risk suspension scenarios)
...
| Trading / Risk | 🟡 Pending | Sign off on production vig rate table values; default admin fee; sport / league enable list; **chosen approach for handling line moves on Bets that reference a numeric line** |
...
| Customer Support | 🟡 Pending | New "Cashed Out" status playbook; dispute handling for price-changed-during-validation; **push-risk suspension explanation** |
...
|:---|:---|:---|:---|
| **Phase 1 — V1 Launch** | Cash Out across every enabled market on enabled leagues. Back-office configuration tool (Settings, Templates, Customer Manager). Trading liability reporting. Full widget state coverage. Push-risk suspension. Trading-selected approach for line-move handling on Bets that reference a numeric line. | **Launch tournament kick-off** _(exact date TBD by Trading + Eng joint readiness)_ | Full market parity at launch — no Punter is excluded from cashout based on the market they wagered on |
| **Phase 2 — League Expansion** | Add cashout coverage across all priority leagues that were not enabled in Phase 1. The same pricing logic, configuration, and reporting from Phase 1 reuses. | **Post-launch tournament** | Maximum league coverage across every market |
...
#### 🎬 Use Case 3: Cash Out a Bet on a Market With Multiple Selections When Price Changes UP During Validation

| | |
|:---|:---|
| **👤 Actor** | Punter with a pending Bet on a market that has more than two selections, on a league enabled in the back office |
| **🎯 Goal** | Cash out and have the system handle a favourable price move automatically, with the formula computing correctly across all selections in the market |

<details>
<summary><strong>📖 Scenario</strong> — click to expand</summary>

1. Punter has $50 on **Selection A** in a market with three selections (A, B, C). Cashout offer reads "Cash Out $42.30".
2. Punter clicks Cash Out. Validation begins.
3. **During validation, Selection A's price moves up.** Cashout offer recalculates to $48.20 using the same formula with the new market state. Total Implied is calculated across **all** selections in the market regardless of count.
...
7. Trading sees the liability update across every selection in the market in the platform's liability report.
...
#### 🎬 Use Case 6: Cash Out a Bet Whose Reference Line Has Moved

| | |
|:---|:---|
| **👤 Actor** | Punter with a pending Bet on a market that references a numeric line, where the platform's current main line is different from the line at Bet placement |
...
6. Trading liability update only fires if cashout completes — same liability-report flow as every other market.

</details>

---

#### 🎬 Use Case 7: Cash Out Suspended on a Bet at Risk of Push

| | |
|:---|:---|
| **👤 Actor** | Punter with a pending Bet on a market where a "push" outcome is possible (the live event-state has reached the numeric line that defines a no-win-no-loss settlement) |
...
1. Punter has a pending Bet that depends on a numeric outcome (e.g. score margin) being above or below a specific line.
...
6. Within minutes, the customer-site widget begins offering Cash Out on Bets in this league using the new template's pricing — across every enabled market on that league. The Calculator tab uses this template to validate offer amounts ad-hoc.
...
- ❌ **Multi-Bet cashout** (Bets composed of multiple selections from different markets) — single-Bet only in V1. Multi-leg pricing requires a separate model.
...
|:--:|:---|:---|:---|
| 1 | **Cashout offer formula** with two price-state branches: when the market price equals the placement price → `Cashout = Wager × (1 − AdminFee)`. When the market price differs → `Cashout = (InitialOdd / TruePropOdd) × Wager × (1 − VigRate)`, with vig looked up from the template's vig table by `% Bet Change Win/Lose`. The same formula applies to every Bet regardless of when it was placed, when it is cashed out, or which market it was placed on. | Foundation of all cashout pricing. The two branches are determined by **price state**, not bet timing — one formula across every market. | Trading + Eng (Customer Site) |
| 2 | **All enabled markets supported regardless of selection count**. Total Implied is computed across all selections in the market. | The formula must work for every market the platform offers; no Punter is excluded based on the market they wagered on. | Eng (Customer Site) + UX |
| 3 | **Original-line price resolution** for Bets on markets that reference a numeric line that can shift between Bet placement and cashout. Implementation of the Trading-selected approach: (a) query the odds feed for the original-line price; or (b) suspend cashout while the original line is no longer the platform's main line. The same approach is applied uniformly across V1. | Without this, cashout cannot operate correctly on markets where the line has moved — the formula has no valid `New Odd` for the original line. | Trading + Eng (Trading Tools) |
| 4 | **Push-risk suspension**: when the live result of a market reaches a value equal to the line that would settle the Bet as a push, cashout is suspended for that Bet until the result moves off the line or the event grades. | Protects the Punter from accepting a cashout offer that ignores the push outcome; protects the platform from having to model the three-way (win / push / lose) outcome inside the cashout formula. | Trading + Eng (Customer Site) |
| 5 | **Back office — Settings Manager**: brand / market / sport / league hierarchy. **One Cashout-enabled toggle per league**. Template assignment per league. Edit-mode gating with Save / Cancel. The same configuration applies to every Bet on that league regardless of market or timing. | Trading needs a single surface to enable / disable cashout per league and assign templates. | Eng (Back Office) + Trading Ops |
...
| 13 | **Trading liability reporting** for cashed-out Bets. The same liability flow applies to every Bet regardless of placement timing, cashout timing, or market. The cashed-out bet's payout is replaced by the cashout amount; Profit, Payout, and Volume update on every selection in the market; Risk and Count are unchanged. Wager Coverage shows a Cash Out icon. Wager Details Status = "Cashed Out", Paid = cashout amount. | Trading cannot hedge without real-time liability visibility. A single reporting surface avoids ambiguity about where a given Bet shows up. | Eng (Trading Tools) + Trading |
| 14 | **Cashout button suspended** when no current price for the market, when the market is suspended, when the original line is unavailable per the chosen line-move approach, or when the Bet is in a push-risk window | Without a usable price or a clear settlement state, the formula cannot evaluate — must not offer a stale or guessed price | Eng (Customer Site) + Eng (Trading Tools) |
...
|:---|:---|
| Multi-Bet cashout (Bets across multiple selections / markets) | Multi-leg pricing logic + per-leg vig is a separate model |
...
- The same back-office configuration applies to every Bet regardless of placement timing or market — Trading does not need separate templates, sport toggles, or vig tables for different timing or market types.
- The platform's liability reporting flow applies to every cashed-out Bet regardless of placement, cashout timing, or market.
- The same cashout formula applies regardless of placement, cashout timing, or market — only price state matters.
...
**Description.** Combined decision diagram for Bets on markets that reference a numeric line:
...
|:---|:---|:---|:---|
| **Year 1 — Near-term** | **Phase 1:** Cash Out V1 across every enabled market on enabled leagues (launch tournament anchor) | Trading sign-off on vig table + admin fee; Trading-selected line-move-handling approach; odds feed for enabled leagues (with alt-line support if that approach is chosen); back-office Eng delivery; liability reporting integration | 🟡 Drafting Scope |
